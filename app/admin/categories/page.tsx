...
export default async function AdminCategoriesPage() {
  ...
  async function update(fd: FormData) {
    "use server"
    await updateCategoryAction(fd.get("id") as string, fd)
  }
  ...
  return (
    <div>
      <AdminPageHeader
        eyebrow="Art Store"
        title="Categories"
        description="Categories used to organize products in the Art Store. Deleting a category leaves its products uncategorized rather than deleting them."
      />
      ...
      <section className="card" style={{ padding: "var(--space-5)", marginBottom: "var(--space-7)" }}>
        ...
        <form action={createCategoryAction} style={{ display: "flex", gap: "var(--space-3)", flexWrap: "wrap", alignItems: "flex-end" }}>
          <div className="field" style={{ flex: "1 1 200px" }}>
            <label className="field__label" htmlFor="name">Name</label>
            <input id="name" name="name" type="text" className="input" required placeholder="e.g. Ceramics" />
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-3)" }}>
          {categories.map(c => (
            <form key={c.id} action={update} className="card" style={{ padding: "var(--space-4)", display: "flex", gap: "var(--space-3)", flexWrap: "wrap", alignItems: "flex-end" }}>
              ...
              <span className="chip chip--neutral" style={{ marginBottom: "var(--space-1)" }}>{c._count.products} product{c._count.products === 1 ? "" : "s"}</span>
              <div style={{ display: "flex", gap: "var(--space-2)" }}>
                <button type="submit" className="btn btn--ghost btn--sm">Save</button>
                <DeleteButton action={deleteCategoryAction} id={c.id} itemName={c.name} />
              </div>
            </form>
          ))}
        </div>
